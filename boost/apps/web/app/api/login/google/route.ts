...
import { googleAuth, lucia } from '@/auth/adapter';
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    await db.$transaction(async (trx) => {
      ...
      if (!user) {
        const createdUser = await trx.user.create({
          data: {
            id: googleData.id.toString(), // TODO: Need confirmation if this would do or should be use a different id here
    ...
    // TODO: Alternatively we can give this control to the client side
    return NextResponse.redirect(
